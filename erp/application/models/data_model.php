...
//获取订单单据列表
	public function get_purchaseadvance($where='',$type=2) {
	    $sql = 'select 
		            a.*,
					b.name as contactName,b.number as contactNo,   
					c.number as salesNo ,c.name as salesName, 
					d.number as accountNumber ,d.name as accountName
				from '.$this->db->dbprefix('purchaseadvance').' as a 
					left join 
						(select id,number, name from '.$this->db->dbprefix('contact').' where isDelete=0) as b
					on a.buId=b.id 
					left join 
	...
	//获取订单明细
	public function get_purchaseadvance_info($where='',$type=2) {
	    $sql = 'select 
		            a.*, 
				...
				from '.$this->db->dbprefix('purchaseadvance_info').' as a 
					left join 
