...
class InvwwOi extends CI_Controller {
	...
	public function index() {
		...
		switch ($action) {
			case 'initOi':
			    ...
			    $this->load->view('scm/invOi/initwwOi-'.$type);	
				break;  
			case 'editOi':
	...
	public function listIn() {
		...
		$where = '(a.isDelete=0) and a.billType="WWOI"';
	...
	public function exportInvOi() { 
		$name = 'wwrk_record_'.date('YmdHis').'.xls';
		sys_csv($name);
	...
	public function queryTransType(){
	    $type   = $this->input->get_post('type',TRUE) == 'wwout' ? 'wwout' : 'wwin';
	...
	public function updateIn() {
		...
		$data = $this->data_model->get_invoice('a.isDelete=0 and a.id='.$id.' and a.billType="WWOI"',1);
		if (count($data)>0) {
    ...
    public function toOiPdf() {
		...
		$paperType = $this->input->post('paperType',TRUE);
		$data = $this->data_model->get_invoice('a.isDelete=0 and a.id='.$id.' and a.billType="WWOI"',1);  
		if (count($data)>0) { 
			$data['num']    = 53;
			...
			else if($paperType=='pp3')
							{
			    $html2pdf = new HTML2PDF('L', 'B6', 'en');
							}
							else
    ...
    public function deletein() {
	    ...
	    $id   = intval($this->input->get('id',TRUE));
		$data = $this->mysql_model->get_rows('invoice',array('id'=>$id,'billType'=>'WWOI'));  
		if (count($data)>0) {
	...
	public function listout() {
		...
		$locationId   = intval($this->input->get_post('locationId',TRUE));
		$transTypeId  = intval($this->input->get_post('transTypeId',TRUE));
		$where = 'a.isDelete=0 and a.billType="WWOO"';
		$where .= $matchCon  ? ' and a.postData like "%'.$matchCon.'%"' : ''; 
		$where .= $beginDate ? ' and a.billDate>="'.$beginDate.'"' : ''; 
		$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : ''; 
		$where .= $transTypeId>0 ? ' and a.transType='.$transTypeId.'' : ''; 
	...
	private function get_updateOut($id) {
	    $data = $this->data_model->get_invoice('a.isDelete=0 and a.id='.$id.' and a.billType="WWOO"',1);
		if (count($data)>0) {
			$list = $this->data_model->get_invoice_info('a.isDelete=0 and a.iid='.$id.'  order by a.id desc');   
			foreach ($list as $arr=>$row) {
			    $v[$arr]['invSpec']      = $row['invSpec'];
    ...
    public function deleteOut() {
	    ...
	    $id   = intval($this->input->get('id',TRUE));
		$data = $this->mysql_model->get_rows('invoice',array('id'=>$id,'billType'=>'WWOO'));  
		if (count($data)>0) {
			 $data['checked'] >0 && str_alert(-1,'已审核的不可删除'); 
		    $this->db->trans_begin();
			$this->mysql_model->update('invoice',array('isDelete'=>1),array('id'=>$id));   
	...
	private function Oi_validform($data) {
		...
		if ($data['id']>0) {
		    $invoice = $this->mysql_model->get_rows('invoice',array('id'=>$data['id'],'billType'=>'WWOI','isDelete'=>0));  
			count($invoice)<1 && str_alert(-1,'单据不存在、或者已删除');
			$data['checked'] = $invoice['checked'];	
			$data['billNo']  = $invoice['billNo'];	
		} else {
		    $data['billNo']  = str_no('WWRK');    
		}
	...
	private function Oo_validform($data) {
		...
		if ($data['id']>0) {
		    $invoice = $this->mysql_model->get_rows('invoice',array('id'=>$data['id'],'billType'=>'WWOO','isDelete'=>0));  
			count($invoice)<1 && str_alert(-1,'单据不存在、或者已删除');
			$data['checked'] = $invoice['checked'];	
			$data['billNo'] = $invoice['billNo'];	
		} else {
		    $data['billNo'] = str_no('WWCK');    
		}
		...
		foreach ($data['entries'] as $arr=>$row) {
			...
			(float)$row['price'] < 0  && str_alert(-1,'商品销售单价要为数字，请输入有效数字！'); 
			intval($row['locationId']) < 1 && str_alert(-1,'请选择相应的仓库！'); 
			intval($row['locationId']) != 999 && str_alert(-1,'委外单请选择委外仓库仓库！'); 
			!in_array($row['locationId'],$storage) && str_alert(-1,$row['locationName'].'不存在或不可用！');
			...
			if ($system['requiredCheckStore']==1 && $data['id']<1) {  
				if (intval($data['transType'])==150806) {                        
					if (isset($inventory[$row['invId']][$row['locationId']])) {
						$inventory[$row['invId']][$row['locationId']] < (float)$row['qty'] && str_alert(-1,$row['locationName'].$row['invName'].'委外商品库存不足！'); 
					} else {
						str_alert(-1,$row['invName'].'库存不足！');
					}
				}
...
//单个审核   
	public function checkInvOi() {
	    ...
	    $id   = $data['id'];
		//str_alert(-100,$id);  
		$data = $this->mysql_model->get_results('invoice','(id in('.$id.')) and billType="WWOI" and checked=0 and (isDelete=0)');  
		if (count($data)>0) {
			...
			if ($sql) {
				...
				$this->common_model->logs('单：'.$billno.'的单据已被审核！');
				str_alert(200,'编号：'.$billno.'的单据已被审核！'); 
			} 
			str_alert(-1,'审核失败');   
		}
	...
	public function rsBatchCheckInvOi() {
		...
		if (strlen($id)>0) {
		$sql = $this->mysql_model->update('invoice',array('checked'=>0,'checkName'=>''),array('id'=>$id));
		
			if ($sql) {
				
				$this->common_model->logs('单：'.$id.'的单据已被反审核！');
				str_alert(200,'单据已被反审核！'); 
			} 
			str_alert(-1,'反审核失败');  
		}
		str_alert(-1,'请不要进行重复反审核，请核对刷新后再试！'); 
	}
	//单个审核   
	public function checkInvOo() {
	    ...
	    $id   = $data['id'];
		//str_alert(-100,$id);  
		$data = $this->mysql_model->get_results('invoice','(id in('.$id.')) and billType="WWOO" and checked=0 and (isDelete=0)');  
		if (count($data)>0) {
			$sql = $this->mysql_model->update('invoice',array('checked'=>1,'checkName'=>$this->jxcsys['name']),'(id in('.$id.'))'); 
			if ($sql) {
				foreach($data as $arr=>$row) {
