...
class invPuPre extends CI_Controller {
	...
	public function index() {
		...
		switch ($action) {
			case 'initPur':
				...
				$data['billNo'] = str_no('WLRG');
			    $this->load->view('scm/invPre/initPur',$data);	
				break;  
			case 'editPur':
			    $this->common_model->checkpurview(1);
				$id = intval($this->input->get_post('id',TRUE));
				$data['billNo'] = $this->mysql_model->get_row('purchaseadvance',array('id'=>$id,'billType'=>'PURCHASEADVANCE'),'billNo');  
			    $this->load->view('scm/invPre/initPur',$data);	
				break;  	
			case 'initPurList':
			    $this->common_model->checkpurview(1); 
			    $this->load->view('scm/invPre/initPurList');
				break; 
			default: 
			    $this->common_model->checkpurview(1); 
				//str_alert(-1,'123234');
	...
	public function purList() {
		...
		$where = 'a.isDelete=0 and a.billType="PURCHASEADVANCE"'; 
		...
		$list = $this->data_model->get_purchaseadvance($where.' order by '.$order.' limit '.$rows*($page-1).','.$rows);  
		...
		$json['data']['page']        = $page;
		$json['data']['records']     = $this->data_model->get_purchaseadvance($where,3); 
		$json['status']              = 200;
		$json['msg']                 = 'success'; 		
	...
	public function exportinvPu(){
		...
		$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : ''; 
		$where .= $this->common_model->get_admin_purview();
		$data['list'] = $this->data_model->get_purchaseadvance($where.' order by '.$order);  
		$this->load->view('scm/invPre/exportinvPu',$data);	
	}
	...
	public function findUnhxList(){
		...
		$begindate  = str_enhtml($this->input->get_post('beginDate',TRUE));
		$enddate    = str_enhtml($this->input->get_post('endDate',TRUE));
		$where = '(a.billType="PURCHASEADVANCE") and checked=1';
	...
	public function add(){
		...
		if (strlen($data)>0) {
			//str_alert(-1,$data,8000); 
			$data = $this->validform((array)json_decode($data, true));
			//str_alert(-1,$data['id'],8000); 
			/*$info = elements(array(
				'billNo','billType','transType','transTypeName','buId','billDate','hxStateCode','description','totalQty','amount','arrears','rpAmount','totalAmount','totalArrears','disRate','disAmount','postData','salesId','uid','userName','srcOrderNo','srcOrderId','accId','modifyTime'),$data,NULL);*/
			$info = elements(array(
				...
				'totalArrears','disRate','disAmount','salesId','uid','userName','srcOrderNo','srcOrderId',
				'accId','modifyTime'),$data,NULL);
			$this->db->trans_begin();
				//str_alert(-1,$data,8000); 		
			$iid = $this->mysql_model->insert('purchaseadvance',$info);   
			$this->purchaseadvance_info($iid,$data);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->common_model->logs('新增预购单据编号：'.$info['billNo']);//.'，报文：'.$_POST['postData']//stripslashes(json_encode($info))
	...
	public function updateinvPre(){
	    $this->common_model->checkpurview(3);
	    $data = $this->input->post('postData',TRUE);
		if (strlen($data)>0) {
			$data = $this->validform((array)json_decode($data, true));
			$info =  elements(array(
				'billNo','billType','transType','transTypeName','buId','billDate','hxStateCode','description','totalQty','amount','arrears','rpAmount','totalAmount','totalArrears','disRate','disAmount','postData','uid','userName','srcOrderNo','srcOrderId','accId','modifyTime'),$data,NULL);
			$this->db->trans_begin();
			$this->mysql_model->update('purchaseadvance',$info,array('id'=>$data['id']));
			$this->purchaseadvance_info($data['id'],$data);
	...
	public function update() {
	    $this->common_model->checkpurview(1);
	    $id   = intval($this->input->get_post('id',TRUE));
		$data =  $this->data_model->get_purchaseadvance('a.isDelete=0 and a.id='.$id.' and a.billType="PURCHASEADVANCE"',1);
		if (count($data)>0) {
			$info['status'] = 200;
			$info['msg']    = 'success'; 
			...
			$list = $this->data_model->get_purchaseadvance_info('a.isDelete=0 and a.iid='.$id.' order by a.id');  
			foreach ($list as $arr=>$row) {
    ...
    public function toPdf() {
		...
		if (count($data)>0) { 
		    ...
		    ob_start();
			$this->load->view('scm/invPre/toPdf',$data);
    ...
    public function delete() {
		...
		$data = $this->mysql_model->get_results('purchaseadvance','(isDelete=0) and (id in('.$id.')) and billType="PURCHASEADVANCE"');  
		if (count($data)>0) {
		    ...
		    $this->db->trans_begin();
			$this->mysql_model->update('purchaseadvance',array('isDelete'=>1),'(id in('.$id.'))');   
			$this->mysql_model->update('purchaseadvance_info',array('isDelete'=>1),'(iid in('.$id.'))');   
    ...
    public function delete1() {
	    $this->common_model->checkpurview(4);
	    $id   = intval($this->input->get('id',TRUE));
		$data = $this->mysql_model->get_rows('purchaseadvance',array('id'=>$id,'billType'=>'PURCHASEADVANCE'));  
		if (count($data)>0) {
		    //$data['checked'] >0 && str_alert(-1,'已审核的不可删除'); 
		    $this->db->trans_begin();
			$this->mysql_model->update('purchaseadvance',array('isDelete'=>1),array('id'=>$id));   
			$this->mysql_model->update('purchaseadvance_info',array('isDelete'=>1),array('iid'=>$id));  
			if ($data['accId']>0) { 
	...
	//批量审核  
    public function batchCheckinvPre() {
	    $this->common_model->checkpurview(86);
	    $id   = str_enhtml($this->input->post('id',TRUE));
		$data = $this->mysql_model->get_results('purchaseadvance','(id in('.$id.')) and billType="PURCHASEADVANCE" and isDelete=0');  
		if (count($data)>0) {
		    foreach($data as $arr=>$row) {
			...
			$sql = $this->mysql_model->update('purchaseadvance',array('checked'=>1,'checkName'=>$this->jxcsys['name']),'(id in('.$id.'))'); 
			if ($sql) {
			    $this->mysql_model->update('purchaseadvance_info',array('checked'=>1),'(iid in('.$id.'))'); 
	...
	//批量反审核
    public function rsBatchCheckinvPre() {
		...
		if (count($data)>0) {
			...
			$sql = $this->mysql_model->update('purchaseadvance',array('checked'=>0,'checkName'=>''),'(id in('.$id.'))'); 
			if ($sql) {
			    $this->mysql_model->update('purchaseadvance_info',array('checked'=>0),'(iid in('.$id.'))'); 
	...
	private function validform($data) {
		...
		if ($data['id']>0) {
		    $purchaseadvance = $this->mysql_model->get_rows('purchaseadvance',array('id'=>$data['id'],'billType'=>'PURCHASEADVANCE','isDelete'=>0));  
			count($purchaseadvance)<1 && str_alert(-1,'单据不存在、或者已删除');
			$data['checked'] = $purchaseadvance['checked'];	
			$data['billNo']  = $purchaseadvance['billNo'];	
		} else {
		    //$data['billNo']  = str_no('CGRG');    
		}
		...
		/*if ($system['requiredCheckStore']==1) {
		    $inventory = $this->data_model->get_purchaseadvance_info_inventory();
		}*/
	...
	private function purchaseadvance_info($iid,$data) 
    {
		foreach ($data['entries'] as $arr=>$row) {
			$v[$arr]['iid']              = $iid;
			$v[$arr]['uid']              = $data['uid'];
		...
		//str_alert(-1,$v[$arr]['id'],8000); 
		if (isset($v)) {
			if (isset($data['id']) && $data['id']>0)
				{  
			    $this->mysql_model->delete('purchaseadvance_info','(iid='.$iid.')');
				}
			$this->mysql_model->insert('purchaseadvance_info',$v);
		}
	...
	public function getImagesById() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['status']       = 1;
			$v[$arr]['name']         = $row['name'];
			$v[$arr]['url']          = site_url().'/scm/invPre/getImage?action=getImage&pid='.$row['id'];
			$v[$arr]['thumbnailUrl'] = site_url().'/scm/invPre/getImage?action=getImage&pid='.$row['id'];
			$v[$arr]['deleteUrl']    = '';
			$v[$arr]['deleteType']   = '';
		}
		$json['status'] = 200;
	...
	public function uploadImages() {
		...
		$files[0]['url']          = site_url().'/scm/invPre/getImage?action=getImage&pid='.$newid;
		$files[0]['thumbnailUrl'] = site_url().'/scm/invPre/getImage?action=getImage&pid='.$newid;
		$files[0]['deleteUrl']    = '';
		$files[0]['deleteType']   = '';
		$json['status'] = 200;
		$json['msg']    = 'success';
