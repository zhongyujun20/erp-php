...
// 			die('{"status":200,"msg":"success","data":{"linkMen":'.$data['linkMans'].',"contactName":"'.$data['name'].'","buId":'.$data['id'].',"cLevel":0}}');
	...
	public function add(){
		...
		if ($sql) {
			$data['id'] = $sql;
			$username = $this->jxcsys['username'];
			$data2 = $this->mysql_model->get_rows('admin',array('username'=>$username));
			if (count($data2)>0) {
			    $type = $data['type']== -10 ? 2 : 4;
			    $array  = explode(',',$data2['righttype'.$type]);
		        $newid = array($sql);
		        $info['righttype'.$type] = join(',',array_filter(array_merge($array,$newid)));
