...
class Inventory extends CI_Controller {
	...
	public function index() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['dopey']  = $row['dopey'];
			$list1= $this->mysql_model->get_row('goods_img',array('invId'=>$row['id'],'isDelete'=>0),'url');
			$v[$arr]['url']     = count($list1)>0 ? $list1 :'';
		}
		$json['status'] = 200;
		$json['msg']    = 'success'; 
		$json['data']['page']      = $page;                                                            
	...
	private function validform($data) {
	   ...
	   if(strlen($data['spec']) < 1)
		{
		$specname = $this->mysql_model->get_row('goods',array('name'=>$data['name'],'isDelete'=>0),'spec');	
		strlen($specname) < 1&& str_alert(-1,'商品名称已存在，规格不能为空且不能为'.$specname);
		}
