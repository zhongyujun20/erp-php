...
class Right extends CI_Controller {
	...
	public function isMaxShareUser() {
		die('{"status":200,"data":{"totalUserNum":2000,"shareTotal":1},"msg":"success"}');	
	}
	...
	public function auth2User(){
		...
		if (count($data)>0) {
			$userName == 'admin' && str_alert(-1,'管理员不可操作');  
			$result1=$this->mysql_model->get_count('admin',array('status'=>1));
			if($result1<5)
			{
