		...
		loadGrid: function(a) {
			...
			w.mod_PageConfig.gridReg(C, B), B = w.mod_PageConfig.conf.grids[C].colModel, $("#grid").jqGrid({
				...
				loadComplete: function(a) {
					if (THISPAGE.$_barCodeInsert = $("#barCodeInsert"), urlParam.id > 0||urlParam.preid) {
						var b = a.rows,
							c = b.length;
						w.newId = c + 1;
						for (var d = 0; c > d; d++) {
...
$(function() {
    ...
    else if(urlParam.preid){//预购转移到采购
        var a = $(".bills").hide();
        Public.ajaxGet("../scm/invPuPre/update?action=update", {
            id: urlParam.preid
        }, function(b) {
            b.data.status="add";//直接跳转增加
            b.data.id=-1;//跳转增加页面
            200 === b.status ? (originalData = b.data, THISPAGE.init(b.data), a.show(), hasLoaded = !0) : parent.Public.tips({
                type: 1,
                content: b.msg
