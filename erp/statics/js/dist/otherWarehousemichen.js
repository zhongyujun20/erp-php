var curRow, curCol, loading, urlParam = Public.urlParam(),
	...
	canPrev = false,//add by michen 20190825
	THISPAGE = {
		init: function(a) {
			...
			$.cookie("BarCodeInsert") && THISPAGE.$_barCodeInsert.addClass("active"), this.goodsEdittypeInit()//add by michen 20190825
			,a.id > 0 && a.checked ? (this.disableEdit(),$('#audit').hide(),$("#mark").addClass("has-audit")) : ($('#reAudit').hide(),$("#mark").removeClass("has-audit"), this.editable = !0, $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
			}))
		},
		initDom: function(a) {
		...
		loadGrid: function(a) {
			...
			var z = "grid",
				...
				}, 
				{
					...
					formatter: function(a, b, c) {
						var d = "";
						if(c.url)d = "<img src='"+c.url+"' width='56' style='margin-top:3px'/>";
						return d;
					},
					title: !0
				}, //add by michen 20171208 for 商品图片
				{
					name: "goods",
					label: "商品",
					nameExt: '<span id="barCodeInsert">扫描枪录入</span>',//add by michen 20190825
					width: 320,
					title: !0,
					classes: "goods",
					formatter: b,
					...
					enterCallback: function() {
						if (THISPAGE.$_barCodeInsert.hasClass("active")) {
							...
							;canPrev&&($("#grid").jqGrid("editCell", a-1,3, true),canPrev = false);//add by michen 20180225 for 扫码枪数量合并累计
			...
			v.mod_PageConfig.gridReg(z, A), A = v.mod_PageConfig.conf.grids[z].colModel, $("#grid").jqGrid({
				...
				loadComplete: function(a) {
					THISPAGE.$_barCodeInsert = $("#barCodeInsert");//add by michen 20190825
				...
				beforeSaveCell: function(a, b, c, d, e) {
					if ("goods" === b) {
						...
						f&&$("#grid").jqGrid("setRowData", a, {url: f.url});//add by michen 20171208 for 商品图片
						if (!f) {
							v.skey = c;
							var g, h = function(b) {
									//add by michen 20190825
									//add by michen 20180225 for 扫码枪数量合并累计
									var idAry = $("#grid").jqGrid('getDataIDs');
									for (var d = 0, e = idAry.length; e > d; d++) {
										var gg = idAry[d],
											h = $("#grid").jqGrid("getRowData", gg);
										if(h.unitId){
											var gdnum = h.goods.substr(0,b.number.length);
											if (b.number == gdnum) {
												var i = $("#" + gg).data("goodsInfo");
												$("#grid").jqGrid("setRowData", gg, {qty:(Number(h.qty)+1)});
												canPrev = true;
												var d = parseFloat((Number(h.qty)+1)),
													ggg = parseFloat(h.price),
													h = parseFloat(h.discountRate);
												if ($.isNumeric(ggg)) if ($.isNumeric(h)) var i = d * ggg * h / 100,
													j = d * ggg - i,
													k = $("#grid").jqGrid("setRowData", gg, {
														deduction: i,
														amount: j
													});
												else var k = $("#grid").jqGrid("setRowData", gg, {
													amount: d * ggg
												});
												k && THISPAGE.calTotal();
												g="&nbsp;";//&#160;&nbsp;
		...
		reloadData: function(a) {debugger;
			function b() {
