	...
	THISPAGE = {
		...
		loadGrid: function() {
			...
			queryConditions.beginDate = this.$_beginDate.val(), queryConditions.endDate = this.$_endDate.val(), $("#grid").jqGrid({
				url: "../scm/invwwOi/listOut?action=listOut&type=wwout",
		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				parent.tab.addTabItem({
					...
					text: "委外出库",
					url: "../scm/invwwOi?action=editOi&type=wwout&id=" + b
				});
				...
				if (a.preventDefault(), Business.verifyRight("OO_DELETE")) {
					...
					$.dialog.confirm("您确定要删除该出库记录吗？", function() {
						Public.ajaxGet("../scm/invwwOi/deleteOut?action=deleteOut", {
							id: b
						}, function(a) {
				...
				$(".wrapper").on("click", "#audit", function(a) {
					if (a.preventDefault(), Business.verifyRight("OO_CHECK")) {
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invwwOi/batchCheckInvOo?action=batchCheckInvOo", {
							id: c
						}, function(a) {
							if (200 === a.status) {
					...
					if (a.preventDefault(), Business.verifyRight("OO_UNCHECK")) {
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invwwOi/rsBatchCheckInvOo?action=rsBatchCheckInvOo", {
							id: c
						}, function(a) {
							if (200 === a.status) {
			...
			$("#search").click(function() {
				...
				var f = "../scm/invwwOi/exportInvOo?action=exportInvOo" + d;
				$(this).attr("href", f)
			})
		}
	};
