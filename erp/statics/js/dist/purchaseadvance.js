var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		...
		initDom: function(a) {
			var b = this;
			this.$_customer = $("#customer"), this.$_date = $("#date").val(system.endDate), this.$_number = $("#number"), this.$_note = $("#note"), this.$_discountRate = $("#discountRate"), this.$_deduction = $("#deduction"), this.$_discount = $("#discount"), this.$_payment = $("#payment"), this.$_arrears = $("#arrears"), this.$_totalArrears = $("#totalArrears"), this.$_toolTop = $("#toolTop"), this.$_toolBottom = $("#toolBottom"), this.$_paymentTxt = $("#paymentTxt"), this.$_accountInfo = $("#accountInfo"), this.$_userName = $("#userName"), this.$_modifyTime = $("#modifyTime"), this.$_checkName = $("#checkName"), this.customerArrears = 0, this.$_note.placeholder(),

				// "150502" == originalData.transType ? (parent.$("#page-tab").find("li.l-selected").children("a").html("购货退货单"), $("#paymentTxt").html("本次退款")) :
				// 	(parent.$("#page-tab").find("li.l-selected").children("a").html("采购入库单"), $("#paymentTxt").html("本次付款")),
				this.customerCombo = Business.billSupplierCombo($("#customer"), {}), ("add" !== a.status || a.buId) && (this.$_customer.data("contactInfo", {
				id: a.buId,
				name: a.contactName
			...
			var c = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				d = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invPuPre/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a><a id="edit" class="ui-btn">保存</a>',
				e = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invPuPre/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				f = "",
				g = "";
			// billRequiredCheck ? (f = '<a class="ui-btn" id="audit">审核</a>', g = '<a class="ui-btn" id="reAudit">反审核</a>') : this.$_checkName.parent().hide();
			false ? (f = '<a class="ui-btn" id="audit">审核</a>', g = '<a class="ui-btn" id="reAudit">反审核</a>') : this.$_checkName.parent().hide();
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost("../scm/invPuPre/add?action=add", {
				...
				var c = $(this),
					d = THISPAGE.getPostData();
				d && c.ajaxPost("../scm/invPuPre/addNew?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
					if (200 === b.status) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invPuPre/updateInvPre?action=updateInvPre", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_CHECK")) {
					...
					d && c.ajaxPost("../scm/invPuPre/checkInvPu?action=checkInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_UNCHECK")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invPuPre/revsCheckInvPu?action=revsCheckInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (a.preventDefault(), Business.verifyRight("PU_ADD")) {
					...
					parent.tab.overrideSelectedTabItem({
						tabid: c,
						text: b,
						url: "../scm/invPuPre?action=initPur&transType=" + originalData.transType
					})
				}
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invPuPre/update?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invPuPre/update?action=update", {
					id: a.idList[a.idPostion]
				}, function(b) {
...
$(function() {
	if (urlParam.id) {
		if (!hasLoaded) {
			...
			}) : Public.ajaxGet("../scm/invPuPre/update?action=update", {
				id: urlParam.id
			}, function(b) {
