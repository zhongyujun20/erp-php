var curRow, curCol, curArrears, loading, urlParam = Public.urlParam(),
	...
	THISPAGE = {
		init: function(a) {
			this.mod_PageConfig = Public.mod_PageConfig.init("otherOutbound"), SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_COSTAMOUNT || (hiddenAmount = !0), this.loadGrid(a), this.initDom(a), this.initCombo(), this.addEvent(),a.id > 0 && a.checked ? (this.disableEdit(),$('#audit').hide(),$("#mark").addClass("has-audit")) : ($('#reAudit').hide(),$("#mark").removeClass("has-audit"), this.editable = !0, $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
			}))
		},
		initDom: function(a) {
			...
			this.customerCombo = Business.billCustomerCombo($("#customer"), {
				...
				data: "../scm/invwwOi/queryTransType?action=queryTransType&type=wwout",
				ajaxOptions: {
			...
			var c = "",
				d = "",
				e = "";
			if (billRequiredCheck && (c = '<a class="ui-btn" id="audit">审核</a>', d = '<a class="ui-btn" id="reAudit">反审核</a>'), this.btn_audit = c, this.btn_reaudit = d, this.btn_tempSave = e, this.btn_add = '<a id="savaAndAdd" class="ui-btn ui-btn-sp mrb">保存并新增</a><a id="save" class="ui-btn">保存</a>' + e, this.btn_edit = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invwwOi/toOoPdf?action=toOoPdf&billType=WWOO" target="_blank" id="print" class="ui-btn mrb">打印</a><a id="edit" class="ui-btn">保存</a>', this.btn_view = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invwwOi/toOoPdf?action=toOoPdf&billType=WWOO" target="_blank" id="print" class="ui-btn mrb">打印</a>', this.btn_p_n = '<a class="ui-btn-prev mrb" id="prev" title="上一张"><b></b></a><a class="ui-btn-next" id="next" title="下一张"><b></b></a>', a.id > 0) {
		...
		addEvent: function() {
			...
			this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function(a) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/invwwOi/addOo?action=addOo&type=wwout", {
				...
				if (b.preventDefault(), Business.verifyRight("OO_UPDATE")) {
					...
					c && Public.ajaxPost("../scm/invwwOi/updateOo?action=updateOo&type=wwout", {
				...
				if (b.preventDefault(), Business.verifyRight("OO_CHECK")) {
					var c = THISPAGE.getPostData();
					c && Public.ajaxPost("../scm/invwwOi/checkInvOo?action=checkInvOo", {
						postData: JSON.stringify(c)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("OO_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invwwOi/rsBatchCheckInvOo?action=rsBatchCheckInvOo", {
						id: originalData.id
					}, function(b) {
				...
				var d = THISPAGE.getPostData();
				d && (c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/invwwOi/addNewOo?action=addNewOo&type=wwout", {
					postData: JSON.stringify(d)
				}, function(b) {
				...
				a.preventDefault(), Business.verifyRight("OO_ADD") && parent.tab.overrideSelectedTabItem({
					...
					text: "委外出库",
					url: "../scm/invwwOi?action=initOi&type=wwout"
				})
			}), $(".wrapper").on("click", "#print", function(a) {
				a.preventDefault(), Business.verifyRight("OO_PRINT") && Public.print({
					title: "销货单列表",
					$grid: $("#grid"),
					pdf: "../scm/invwwOi/toOoPdf?action=toOoPdf",
					billType: 10201,
					filterConditions: {
						id: originalData.id
					}
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invwwOi/updateOut?action=updateOut&type=wwout", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invwwOi/updateOut?action=updateOut&type=wwout", {
					id: a.salesListIds[a.idPostion]
				}, function(a) {
		...
		getPostData: function(a) {
			...
			if (f.length > 0) {
				...
				var h = {
					...
					description: g === b.$_note[0].defaultValue ? "委外出库" : g
				};
				return h
			}
...
$(function() {
	if (urlParam.id) hasLoaded || Public.ajaxGet("../scm/invwwOi/updateOut?action=updateOut&type=wwout", {
		id: urlParam.id
	}, function(a) {
