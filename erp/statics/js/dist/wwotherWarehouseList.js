	...
	THISPAGE = {
		...
		loadGrid: function() {
			...
			queryConditions.beginDate = this.$_beginDate.val(), queryConditions.endDate = this.$_endDate.val(), $("#grid").jqGrid({
				url: "../scm/invwwOi/listIn?action=listIn&type=wwin",
		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				parent.tab.addTabItem({
					...
					text: "委外入库",
					url: "../scm/invwwOi?action=editOi&type=wwin&id=" + b
				});
				...
				if (a.preventDefault(), Business.verifyRight("IO_DELETE")) {
					...
					$.dialog.confirm("您确定要删除该入库记录吗？", function() {
						Public.ajaxGet("../scm/invwwOi/deleteIn?action=deleteIn", {
							id: b
						}, function(a) {
				...
				$(".wrapper").on("click", "#audit", function(a) {
					if (a.preventDefault(), Business.verifyRight("IO_CHECK")) {
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invwwOi/batchCheckInvOi?action=batchCheckInvOi", {
							id: c
						}, function(a) {
							if (200 === a.status) {
					...
					if (a.preventDefault(), Business.verifyRight("IO_UNCHECK")) {
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invwwOi/rsBatchCheckInvOi?action=rsBatchCheckInvOi", {
							id: c
						}, function(a) {
							if (200 === a.status) {
			...
			$("#search").click(function() {
				...
				var f = "../scm/invwwOi/exportInvOi?action=exportInvOi" + d;
				$(this).attr("href", f)
			})
		}
	};
