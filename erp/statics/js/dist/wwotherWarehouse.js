var curRow, curCol, loading, urlParam = Public.urlParam(),
	...
	THISPAGE = {
		init: function(a) {
			this.mod_PageConfig = Public.mod_PageConfig.init("wwotherWarehouse"), SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_INAMOUNT || (hiddenAmount = !0), this.loadGrid(a), this.initDom(a), this.initCombo(), this.addEvent(),a.id > 0 && a.checked ? (this.disableEdit(),$('#audit').hide(),$("#mark").addClass("has-audit")) : ($('#reAudit').hide(),$("#mark").removeClass("has-audit"), this.editable = !0, $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
			}))
		},
		initDom: function(a) {
			...
			this.customerCombo = Business.billSupplierCombo($("#customer"), {
				...
				data: "../scm/invwwOi/queryTransType?action=queryTransType&type=wwin",
				ajaxOptions: {
		...
		addEvent: function() {
			...
			this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function(a) {
				...
				c && ("edit" === originalData.stata && (c.id = originalData.id, c.stata = "edit"), Public.ajaxPost("../scm/invwwOi/add?action=add&type=wwin", {
				...
				if (b.preventDefault(), Business.verifyRight("IO_UPDATE")) {
					var c = THISPAGE.getPostData();
					c && Public.ajaxPost("../scm/invwwOi/updateOi?action=updateOi&type=wwin", {
						postData: JSON.stringify(c)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("IO_CHECK")) {
					var c = THISPAGE.getPostData();
					c && Public.ajaxPost("../scm/invwwOi/checkInvOi?action=checkInvOi", {
						postData: JSON.stringify(c)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("IO_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invwwOi/rsBatchCheckInvOi?action=rsBatchCheckInvOi", {
						id: originalData.id
					}, function(b) {
				...
				b.preventDefault();
				var c = THISPAGE.getPostData();
				c && Public.ajaxPost("../scm/invwwOi/addNew?action=addNew&type=wwin", {
					postData: JSON.stringify(c)
				}, function(b) {
					if (200 === b.status) {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invwwOi/updateIn?action=updateIn&type=wwin", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invwwOi/updateIn?action=updateIn&type=in", {
					id: a.salesListIds[a.idPostion]
				}, function(a) {
		...
		getPostData: function() {
			...
			if (e.length > 0) {
				...
				var g = {
					...
					description: f === a.$_note[0].defaultValue ? "委外入库" : f
				};
				return g
			}
	...
	originalData;
$(function() {
	if (urlParam.id) hasLoaded || Public.ajaxGet("../scm/invwwOi/updateIn?action=updateIn&type=wwin", {
		id: urlParam.id
	}, function(a) {
